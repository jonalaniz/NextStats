...
open class XMLEncoder {
    ...
    public enum KeyEncodingStrategy {
        ...
        /// `CharacterSet.uppercaseLetters` and
        ...
        /// (in case you need to locate this key within the payload). The returned
        /// key is used in place of the last component in the coding path before
        /// encoding.
        ...
        internal static func _convertToSnakeCase(_ stringKey: String) -> String {
            ...
            // The general idea of this algorithm is to split
            // words on transition from lower to upper case,
            // then on transition of >1 upper case characters to
            // lowercase
            //
            // myProperty -> my_property
            // myURLProperty -> my_url_property
            //
            // We assume, per Swift naming conventions, that the
            // first character of the key is lowercase.
            ...
            while let upperCaseRange = stringKey.rangeOfCharacter(
                ...
                // Is the next lowercase letter more than 1
                // after the uppercase? If so, we
                // encountered a group of uppercase letters
                ...
                if lowerCaseRange.lowerBound == nextCharacterAfterCapital {
                    ...
                    // There was a range of >1 capital
                    // letters. Turn those into a word,
                    // stopping at the capital before the
    ...
    /// - parameter withRootKey: the key used to wrap the encoded
    /// values.
    ...
    open func encode<T: Encodable>(
        ...
        if topLevel is NSNull {
            ...
            throw EncodingError.invalidValue(
                ...
                EncodingError.Context(
                    codingPath: [],
                    debugDescription: "Top-level \(T.self) encoded as number XML fragment."
                )
            )
            ...
            throw EncodingError.invalidValue(
                ...
                EncodingError.Context(
                    codingPath: [],
                    debugDescription: "Top-level \(T.self) encoded as string XML fragment."
                )
            )
        }
        ...
        return element.toXMLString(
            with: header,
            withCDATA: stringEncodingStrategy != .deferredToString).data(
                using: .utf8, allowLossyConversion: true
            )!
    }
}
...
internal class _XMLEncoder: Encoder {
    ...
    internal var canEncodeNewValue: Bool {
        // Every time a new value gets encoded, the key it's
        // encoded for is pushed onto the coding path (even
        // if it's a nil key from an unkeyed container).
        // At the same time, every time a container is
        // requested, a new value gets pushed onto the
        // storage stack.
        // If there are more values on the storage stack than
        // on the coding path, it means the value is
        // requesting more than one container, which
        // violates the precondition.
        //
        // This means that anytime something that can request
        // a new container goes onto the stack, we MUST push
        // a key onto the coding path.
        // Things which will not request containers do not
        // need to have the coding path extended for them
        // (but it doesn't matter if it is, because they
        // will not reach here).
    ...
    public func unkeyedContainer() -> UnkeyedEncodingContainer {
        ...
        if self.canEncodeNewValue {
            ...
            guard let container = self.storage.containers.last as? NSMutableArray
            else {
                preconditionFailure(
...
private struct _XMLKeyedEncodingContainer<K: CodingKey>: KeyedEncodingContainerProtocol {
    ...
    public mutating func nestedContainer<NestedKey>(
        ...
        let container = _XMLKeyedEncodingContainer<NestedKey>(
    ...
    public mutating func superEncoder() -> Encoder {
        return _XMLReferencingEncoder(
            referencing: self.encoder,
            key: _XMLKey.super,
            convertedKey: _converted(_XMLKey.super),
            wrapping: self.container
        )
    }

    public mutating func superEncoder(
        forKey key: Key
    ) -> Encoder {
        return _XMLReferencingEncoder(
            referencing: self.encoder,
...
private struct _XMLUnkeyedEncodingContainer: UnkeyedEncodingContainer {
    ...
    public mutating func encode(_ value: Float)  throws {
        // Since the float may be invalid and throw, the coding
        // path needs to contain this key.
    ...
    public mutating func encode(_ value: Double) throws {
        // Since the double may be invalid and throw, the coding
        // path needs to contain this key.
        self.encoder.codingPath.append(_XMLKey(index: self.count))
        defer { self.encoder.codingPath.removeLast() }
        self.container.add(try self.encoder.box(value))
...
extension _XMLEncoder {
    ...
    fileprivate func box<T: Encodable>(
        _ value: T
    ) throws -> NSObject {
        return try self.box_(value) ?? NSDictionary()
    }

    // This method is called "box_" instead of "box" to
    // disambiguate it from the overloads. Because the return
    // type here is different from all of the "box" overloads
    // (and is more general), any "box" calls in here would call
    // back into "box" recursively instead of calling the
