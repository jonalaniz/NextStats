...
internal struct _XMLKeyedDecodingContainer<K: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    internal init(
        referencing decoder: _XMLDecoder,
        ...
        case .convertFromSnakeCase:
            // Convert the snake case keys in the container to
            // camel case.
            // If we hit a duplicate key after conversion, then we'll
            // use the first one we saw. Effectively an undefined
    ...
    public func decode(_ type: Bool.Type, forKey key: Key) throws -> Bool {
        ...
        guard let value = try self.decoder.unbox(entry, as: Bool.self)
        else {
    ...
    public func decode(_ type: Int.Type, forKey key: Key) throws -> Int {
        ...
        guard let value = try self.decoder.unbox(entry, as: Int.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public func decode(
        ...
        guard let value = try self.decoder.unbox(entry, as: Int8.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public func decode(
        ...
        guard let value = try self.decoder.unbox(entry, as: Int16.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
    ...
    public func decode(
        ...
        guard let value = try self.decoder.unbox(entry, as: Int32.self)
        else {
            throw DecodingError.valueNotFound(
                type,
                DecodingError.Context(
